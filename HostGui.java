...
public class HostGui{
	...
	keywordField.addActionListener(eventListener);
	...
	commandField.addActionListener(eventListener);
	...
	public void actionPerformed (final ActionEvent e) {
		...
		if (btnConnect.getText().equals("Disconnect")) {
		    ...
		    try
			{
			    ...
			    speed = "" + comboBox.getSelectedItem();
			    ...
			    commandArea.append("Please enter the name of the file with the file descriptions.\n");
			    uploadFlag = false;
			}
		    catch (IOException ioEx)
			{
	    ...
	    if (e.getSource().equals(btnSearch) || e.getSource().equals(keywordField)) {
	    ...
	    if (e.getSource().equals(btnGo) || e.getSource().equals(commandField)) {
		if (!uploadFlag){
		    try
			{
			    ...
			    String filename = commandField.getText();
			    ServerSocket welcomeData = new ServerSocket(port);
			    Socket dataSocket = welcomeData.accept();

			    DataOutputStream dataOutToServer = new DataOutputStream(dataSocket.getOutputStream());
			    dataOutToServer.writeBytes(username + " " + speed + " " + filename + "\n");
